Feature: Model Context Protocol (MCP) support for AI-powered tagging
  ...
  I want FileTagger to support MCP for AI integration
  ...
  Scenario: AI-powered auto-tagging
    ...
    Then FileTagger uses an MCP-compatible AI model to analyze the content
  ...
  Scenario: Semantic search with AI
    ...
    Then FileTagger uses an MCP-compatible AI model to interpret the query
  ...
  Scenario: AI plugin integration
    ...
    Then FileTagger can invoke the plugin for specific automation tasks
  ...
  Scenario: Extensible AI model support
    ...
    When I configure FileTagger to use a new MCP endpoint
