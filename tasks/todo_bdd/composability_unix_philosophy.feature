Feature: Composability and Unix philosophy for CLI
  ...
  So that I can compose FileTagger with other tools and use it in system-level automation

  Scenario: Pipe input and output between tools
    Given a list of files or tags from another tool via stdin
    When I run a FileTagger command that reads from stdin
  ...
  Scenario: JSON output for scripting
    Given I run a FileTagger command with --json
  ...
  Scenario: Quiet mode for automation
    Given I run a FileTagger command with --silent or --quiet
