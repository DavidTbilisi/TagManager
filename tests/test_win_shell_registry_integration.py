...
* **TestWinShellRegistryProbe** — always runs on Windows.
  Writes only under ``HKCU\\Software\\FileTagger\\TestProbes\\<uuid>`` so
...
class TestWinShellRegistryProbe(unittest.TestCase):
    """Writes only under HKCU\\Software\\FileTagger\\TestProbes — Explorer ignores this tree."""

    def setUp(self):
        self._probe = rf"Software\FileTagger\TestProbes\{uuid.uuid4()}"

    def tearDown(self):
        import winreg
        from filetagger.win_context_menu import _reg_delete_subtree
    ...
    def test_cascade_parent_empty_default_and_muiverb_roundtrip(self):
        ...
        wcm._reg_set(winreg.HKEY_CURRENT_USER, parent, "MUIVerb", "FileTagger")
        ...
        self.assertEqual(muiverb, "FileTagger", "MUIVerb carries the visible menu label")
    ...
    def test_delete_subtree_cleans_all_children(self):
        ...
        import winreg
        from filetagger.win_context_menu import _reg_delete_subtree, _reg_set
...
class TestWinShellExplorerKeysE2E(unittest.TestCase):
    ...
    When this class is green the exact keys Explorer reads are verified:
      • Parent keys: (Default)="" + MUIVerb="FileTagger"
    ...
    def test_parent_file_scope_has_empty_default_and_muiverb(self):
        ...
        d, m = wcm.cascade_parent_registry_state_for_tests(winreg.HKEY_CURRENT_USER, parent)
        self.assertEqual((d or ""), "", f"*\\shell\\FileTagger (Default) must be empty; got {d!r}")
        self.assertEqual(m, "FileTagger", f"*\\shell\\FileTagger MUIVerb; got {m!r}")
    ...
    def test_parent_dir_scope_has_empty_default_and_muiverb(self):
        ...
        d, m = wcm.cascade_parent_registry_state_for_tests(winreg.HKEY_CURRENT_USER, parent)
        self.assertEqual((d or ""), "", f"Directory\\shell\\FileTagger (Default) must be empty; got {d!r}")
        self.assertEqual(m, "FileTagger", f"Directory\\shell\\FileTagger MUIVerb; got {m!r}")
    ...
    def test_all_file_scope_leaves_present(self):
        ...
        self.assertEqual(set(subkeys), expected,
            f"Unexpected verbs under *\\shell\\FileTagger\\ExtendedSubCommandsKey\\Shell")
    ...
    def test_all_dir_scope_leaves_present(self):
        ...
        self.assertEqual(set(subkeys), expected,
            f"Unexpected verbs under Directory\\shell\\FileTagger\\ExtendedSubCommandsKey\\Shell")
