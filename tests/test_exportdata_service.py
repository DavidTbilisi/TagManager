...
class TestExportPathRewrite(unittest.TestCase):
    ...
    def test_export_json_relative_to(self):
        from filetagger.app.exportdata.service import export_tags_json

        root = os.path.join(self.tmp, "proj")
        inner = os.path.join(root, "src", "a.py")
        fake = {inner: ["t1"]}
        out = os.path.join(self.tmp, "out.json")
        with patch("filetagger.app.exportdata.service.load_tags", return_value=fake):
    ...
    def test_default_tag_backup_path_next_to_db(self):
        from filetagger.app.exportdata.service import default_tag_backup_path

        fake_db = os.path.join(self.tmp, "home", "file_tags.json")
        with patch("filetagger.app.exportdata.service.get_tag_file_path", return_value=fake_db):
    ...
    def test_backup_tag_database_delegates_to_export(self):
        from filetagger.app import exportdata
        from filetagger.app.exportdata.service import backup_tag_database
    ...
    def test_restore_tag_database_delegates_to_import(self):
        from filetagger.app import exportdata
        from filetagger.app.exportdata.service import restore_tag_database
