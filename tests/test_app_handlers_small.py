...
class TestPathsHandlerAndService(unittest.TestCase):
    @patch("filetagger.app.paths.handler.fuzzy_search_path", return_value="/match")
    @patch("builtins.print")
    def test_handle_path_command_fuzzy(self, mock_print, mock_fuzzy):
        from filetagger.app.paths.handler import handle_path_command
    ...
    @patch("filetagger.app.paths.handler.path_tags", return_value=["a", "b"])
    @patch("builtins.print")
    ...
    @patch("filetagger.app.paths.service.load_tags", return_value={"/abs/x.py": ["t"]})
    @patch("filetagger.app.paths.service.os.getcwd", return_value="/proj")
    @patch("filetagger.app.paths.service.os.path.abspath", return_value="/abs/x.py")
    @patch("filetagger.app.paths.service.os.path.join", return_value="rel")
    @patch("builtins.print")
    def test_path_tags_resolves_and_returns(self, mock_print, mock_join, mock_abspath, mock_cwd, mock_load):
        from filetagger.app.paths.service import path_tags

        out = path_tags("src/x.py")
        self.assertEqual(out, ["t"])
        mock_print.assert_called()

    @patch("filetagger.app.paths.service.load_tags", side_effect=RuntimeError("db"))
    @patch("builtins.print")
    def test_fuzzy_search_path_load_error(self, mock_print, mock_load):
        from filetagger.app.paths.service import fuzzy_search_path

        self.assertEqual(fuzzy_search_path("anything"), "")

    @patch("filetagger.app.paths.service.load_tags", return_value={})
    def test_fuzzy_search_path_empty_db(self, mock_load):
        from filetagger.app.paths.service import fuzzy_search_path

        self.assertEqual(fuzzy_search_path("q"), "")

    @patch("filetagger.app.paths.service.load_tags", return_value={"/a.py": [], "/z.py": []})
    @patch("filetagger.app.paths.service.normalized_levenshtein_distance", return_value=0.9)
...
class TestStorageHandlerAndService(unittest.TestCase):
    @patch("filetagger.app.storage.handler.open_storage_location")
    def test_handle_storage_open(self, mock_open):
        from filetagger.app.storage.handler import handle_storage_command
    ...
    @patch("filetagger.app.storage.handler.show_storage_location", return_value="/tags.json")
    @patch("builtins.print")
    ...
    @patch("filetagger.app.storage.service.get_tag_file_path", return_value=r"C:\data\tags.json")
    def test_show_storage_location(self, mock_gtfp):
        from filetagger.app.storage.service import show_storage_location

        self.assertEqual(show_storage_location(), r"C:\data\tags.json")

    @patch("filetagger.app.storage.service.subprocess.run")
    @patch("filetagger.app.storage.service.get_tag_file_path", return_value="/t.json")
    def test_open_storage_location_darwin(self, mock_gtfp, mock_run):
        from filetagger.app.storage import service as svc
    ...
    @patch("os.startfile")
    @patch("filetagger.app.storage.service.get_tag_file_path", return_value=r"C:\t.json")
...
class TestStatsHandler(unittest.TestCase):
    @patch("filetagger.app.stats.handler.format_namespace_statistics")
    @patch("filetagger.app.stats.handler.get_namespace_statistics", return_value={"n": 1})
    @patch("builtins.print")
    def test_namespaces_branch(self, mock_print, mock_get, mock_fmt):
        from filetagger.app.stats.handler import handle_stats_command

        mock_fmt.return_value = "NS"
        handle_stats_command(namespaces=True)
        mock_print.assert_called_once_with("NS")

    @patch("filetagger.app.stats.handler.format_tag_statistics")
    @patch("filetagger.app.stats.handler.get_tag_statistics", return_value={})
    @patch("builtins.print")
    ...
    @patch("filetagger.app.stats.handler.format_file_count_distribution")
    @patch("filetagger.app.stats.handler.get_file_count_distribution", return_value=[])
    @patch("builtins.print")
    ...
    @patch("filetagger.app.stats.handler.format_overall_statistics")
    @patch("filetagger.app.stats.handler.get_overall_statistics", return_value={})
    @patch("builtins.print")
...
class TestListAllHandler(unittest.TestCase):
    @patch("filetagger.app.list_all.handler.print_list_tags_all_table")
    @patch("builtins.print")
    def test_handle_list_all_command(self, mock_print, mock_table):
        from filetagger.app.list_all.handler import handle_list_all_command
...
class TestVisualizationHandler(unittest.TestCase):
    @patch("filetagger.app.visualization.handler.typer.echo")
    @patch("filetagger.app.visualization.handler.generate_tree_view", return_value="tree")
    def test_handle_tree_view(self, mock_gen, mock_echo):
        from filetagger.app.visualization.handler import handle_tree_view

        handle_tree_view()
        mock_echo.assert_called_once_with("tree")

    @patch("filetagger.app.visualization.handler.typer.echo")
    @patch("filetagger.app.visualization.handler.generate_tag_cloud", return_value="cloud")
    def test_handle_tag_cloud(self, mock_gen, mock_echo):
        from filetagger.app.visualization.handler import handle_tag_cloud

        handle_tag_cloud()
        mock_echo.assert_called_once_with("cloud")

    @patch("filetagger.app.visualization.handler.typer.echo")
    @patch("filetagger.app.visualization.handler.generate_stats_charts", return_value="charts")
    def test_handle_stats_charts(self, mock_gen, mock_echo):
        from filetagger.app.visualization.handler import handle_stats_charts
