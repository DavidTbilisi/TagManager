#!/usr/bin/env python3
"""
FileTagger Configuration Management System
...
class ConfigManager:
    """Manages FileTagger configuration with validation and persistence"""
    ...
    def _get_default_config_dir(self) -> Path:
        ...
        if sys.platform.startswith("win"):
            config_dir = Path.home() / "AppData" / "Local" / "FileTagger"
        else:
            config_dir = Path.home() / ".config" / "filetagger"
        return config_dir
    ...
    def _validate_value(self, key: str, value: Any) -> Any:
        ...
        if expected_type:
            try:
                ...
                raise ValueError(
                    f"Invalid value for {key}: expected {expected_type.__name__}, got {type(value).__name__}"
                )
    ...
    def get_config_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "defaults_available": len(
                [k for k in dir(self._defaults) if not k.startswith("_")]
            ),
        }
