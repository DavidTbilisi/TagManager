...
def render_tree(
    tree: Dict, prefix: str = "", is_last: bool = True, show_tags: bool = True
) -> List[str]:
    """
    ...
    for i, (name, node) in enumerate(items):
        ...
        if node["children"]:
            child_lines = render_tree(
                node["children"], prefix + next_prefix, is_last_item, show_tags
            )
...
def create_tag_cloud_data(
    files_with_tags: Dict[str, List[str]],
...
def render_tag_cloud(
    tag_data: List[Tuple[str, int, float]], width: int = 80
) -> List[str]:
    """
...
def create_ascii_bar_chart(
    data: Dict[str, int], title: str = "Chart", max_width: int = 50
) -> List[str]:
    """
...
def create_ascii_histogram(
    data: List[int], title: str = "Histogram", bins: int = 10
) -> List[str]:
    """
...
def generate_tag_cloud() -> str:
    ...
    if tag_data:
        ...
        lines.append(
            f"📊 Total tag instances: {sum(count for _, count, _ in tag_data)}"
        )
    else:
...
def generate_stats_charts() -> str:
    ...
    lines = ["📊 FileTagger Statistics Charts", "=" * 50, ""]
    ...
    chart2_lines = create_ascii_bar_chart(top_tags, "🏷️  Top 10 Most Used Tags")
    ...
    if len(tags_per_file) > 1:
        chart3_lines = create_ascii_histogram(
            tags_per_file, "📊 Tag Count Distribution Histogram"
        )
    ...
    lines.append(
        f"Most tagged file: {max(len(tags) for tags in files_with_tags.values())} tags"
    )
    lines.append(
        f"Least tagged file: {min(len(tags) for tags in files_with_tags.values())} tags"
    )
