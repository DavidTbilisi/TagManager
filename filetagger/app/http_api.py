...
class _FileTaggerHandler(BaseHTTPRequestHandler):
    server_version = "FileTaggerHTTP/1.0"
...
def run_server(host: str, port: int) -> None:
    httpd = ThreadingHTTPServer((host, port), _FileTaggerHandler)
    print(f"FileTagger HTTP listening on http://{host}:{port}/ (Ctrl+C to stop)")
