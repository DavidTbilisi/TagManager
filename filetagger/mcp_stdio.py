"""
Model Context Protocol (stdio) server for FileTagger.
...
Plugins (Python 3.10+): register callables via entry point group ``filetagger.mcp``.
...
logger = logging.getLogger("filetagger.mcp")
...
def _register_entry_point_plugins(mcp: Any) -> None:
    """Load ``filetagger.mcp`` entry points; each callable is ``register(fastmcp)``."""
    try:
    ...
    try:
        ...
        if hasattr(eps, "select"):
            hooks = eps.select(group="filetagger.mcp")
        elif hasattr(eps, "get"):
            hooks = eps.get("filetagger.mcp", [])
        else:
            hooks = ()
    except Exception as e:
        logger.debug("entry_points filetagger.mcp: %s", e)
        return
...
def run_stdio_server() -> None:
    try:
        ...
        sys.stderr.write(
            "MCP SDK not installed. Use: pip install 'filetagger-cli[mcp]'\n"
            f"Import error: {e}\n"
        )
