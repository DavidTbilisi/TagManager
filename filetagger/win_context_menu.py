...
Registers a single **FileTagger** cascade under:

  HKCU\\Software\\Classes\\*\\shell\\FileTagger\\
  HKCU\\Software\\Classes\\Directory\\shell\\FileTagger\\
...
Launchers live under %%LOCALAPPDATA%%\\FileTagger\\launcher. Explorer passes
...
_LAUNCHER_ROOT = Path(os.environ.get("LOCALAPPDATA", "")) / "FileTagger" / "launcher"

# Single parent folder name under *\shell and Directory\shell
_PARENT_KEY = "FileTagger"
_PARENT_LABEL = "FileTagger"
...
class _Leaf:
    """One Explorer item under the FileTagger cascade."""
...
_LEAVES: Tuple[_Leaf, ...] = (
    ...
    _Leaf("Storage", "Open FileTagger storage folder", "storage", ("file", "dir")),
)
...
def _tm_cli_prefix() -> str:
    """Command prefix to invoke FileTagger CLI (quoted)."""
    exe = shutil.which("ftag")
    if exe:
        return f'"{exe}"'
    py = sys.executable
    return f'"{py}" -m filetagger.cli'
...
def _write_batch(script_stem: str, cli_after_python: str) -> Path:
    ...
    body = (
        ...
        "if not %ERR%==0 (\r\n"
        '  echo.\r\n'
        "  echo FileTagger exited with code %ERR%\r\n"
        "  pause\r\n"
        ")\r\n"
        "endlocal\r\n"
    )
...
def uninstall_context_menu() -> Tuple[int, str]:
    ...
    return 0, "Removed FileTagger cascade from Explorer context menu (current user)."
