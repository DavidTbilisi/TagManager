...
app = typer.Typer(
    name="ftag",
    help="FileTagger - The Ultimate Command-Line File Tagging System",
...
def add(
    ...
    if preset:
        ...
        if preset_tags is None:
            typer.echo(f"Error: preset '{preset}' not found. Use 'ftag preset list' to see available presets.")
            raise typer.Exit(1)
...
def search_main(
    ...
    if tags and path:
        ...
        typer.echo("  ftag search -t python", err=False)
        typer.echo("  ftag search -t python -t web --match-all", err=False)
        typer.echo("  ftag search -t work -x archived        # work, not archived", err=False)
        typer.echo("  ftag search save q1-work -t work -x archived", err=False)
        typer.echo("  ftag search run q1-work", err=False)
        raise typer.Exit(0)
...
def search_list():
    ...
    if not names:
        typer.echo("No saved searches. Use: ftag search save NAME -t ...")
        return
    for n in names:
        typer.echo(f"  {n}")
...
@app.command("mv")
def mv(
    old_path: str = typer.Argument(..., help="Current path tracked in FileTagger"),
...
license_app = typer.Typer(help="Manage your FileTagger Pro license")
shell_app = typer.Typer(help="Internal: shell tag dialog launcher (used by integrations)")
sendto_app = typer.Typer(help="Windows 'Send to' menu integration (right-click -> Send to -> Tag with FileTagger)")
...
def license_activate(
    ...
    """Activate FileTagger Pro with a Paddle transaction id."""
...
def alias_list():
    ...
    aliases = get_aliases()
    if not aliases:
        typer.echo("No aliases configured. Use 'ftag alias add <alias> <canonical>' to add one.")
        return
...
def preset_list():
    ...
    if not presets:
        typer.echo("No presets saved. Use 'ftag preset save <name> --tags ...' to create one.")
        return
...
def backup(
    ...
    """Write a full JSON snapshot of the tag database (same format as ``ftag export``)."""
...
def restore(
    ...
    """Restore tag data from a backup file (merges by default; same rules as ``ftag import``)."""
...
def undo(
    ...
    """Undo recent mutating operations (requires journal: FILETAGGER_JOURNAL=1 or journal.enabled)."""
    if not journal_enabled():
        msg = (
            "Journal is disabled. Set FILETAGGER_JOURNAL=1 or journal.enabled in config."
        )
        if runtime.json_mode():
...
def windows_install_context_menu(
    ...
    """Register FileTagger entries on right-click for files and folders (HKCU)."""
...
def windows_uninstall_context_menu():
    """Remove FileTagger context menu entries installed for the current user."""
...
def mcp_server(
    ...
    """Start the FileTagger MCP server (stdio transport for Claude/AI clients)."""
    import sys
    ...
    try:
        from filetagger.mcp_server.server import main as _mcp_main
        typer.echo("Starting FileTagger MCP server (stdio)…", err=True)
        _mcp_main()
    except ImportError:
...
# admin and the Windows SDK to build/install. The `ftag sendto` and `ftag hotkey`
# commands below give the same convenience without any of that.
#
# `ftag shell tag` remains as the primitive that opens the tag dialog for a
...
def shell_install():
    """[Deprecated] Use `ftag sendto install` instead — Win11 blocks registry-only context menus."""
    print(
        "`ftag shell install` is no longer supported: Windows 11 silently blocks\n"
        ...
        "  ftag sendto install   # adds right-click -> Send to -> Tag with FileTagger\n"
        "  ftag hotkey install   # registers a global Ctrl+Alt+T hotkey"
    )
    raise typer.Exit(1)
...
def shell_uninstall():
    """Remove any registry entries left over from earlier `ftag shell install` attempts."""
...
def sendto_install():
    """Add a 'Tag with FileTagger' entry to the Windows 'Send to' menu (no admin)."""
    raise typer.Exit(handle_sendto_install())


@sendto_app.command("uninstall")
def sendto_uninstall():
    """Remove the 'Tag with FileTagger' Send to entry."""
...
def main():
    """Main entry point for the FileTagger CLI."""
    app()
