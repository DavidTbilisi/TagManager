...
# Import FileTagger services directly (no subprocess overhead)
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
from filetagger.app.add.service import add_tags as _add_tags
from filetagger.app.remove.service import remove_all_tags as _remove_all_tags
from filetagger.app.search.service import search_files_by_tags, combined_search
from filetagger.app.helpers import load_tags, save_tags
from filetagger.app.stats.service import get_overall_statistics

server = Server("filetagger")
...
async def call_tool(name: str, arguments: dict) -> list[types.TextContent]:
    try:
        if name == "tm_add_tags":
            ...
            data = load_tags()
            if path not in data:
                return _err(f"File not found in FileTagger: {path}")
            before = data[path]
            ...
            data = load_tags()
            from filetagger.app.helpers import normalized_levenshtein_distance
            matched: set = set()
